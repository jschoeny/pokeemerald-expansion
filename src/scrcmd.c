...
bool8 ScrCmd_fadescreen(struct ScriptContext *ctx)
{
    ...
    u8 coeffStart = ScriptReadByte(ctx);
    u8 coeffEnd = ScriptReadByte(ctx);

    FadeScreenCoeff(mode, 0, coeffStart, coeffEnd);
...
bool8 ScrCmd_fadescreenspeed(struct ScriptContext *ctx)
{
    ...
    FadeScreenCoeff(mode, speed, coeffStart, coeffEnd);
    SetupNativeScript(ctx, IsPaletteNotActive);
    return TRUE;
}
...
bool8 ScrCmd_givemon(struct ScriptContext *ctx)
{
    ...
    u32 randomize = ScriptReadWord(ctx);
    u32 unkParam2 = ScriptReadWord(ctx);
    u8 unkParam3 = ScriptReadByte(ctx);

    gSpecialVar_Result = ScriptGiveMon(species, level, item, randomize, unkParam2, unkParam3);
    return FALSE;
}
...
{
    u16 species = VarGet(ScriptReadHalfword(ctx));
    u8 randomize = ScriptReadByte(ctx);

    gSpecialVar_Result = ScriptGiveEgg(species, randomize);
    return FALSE;
}
...
bool8 ScrCmd_slatemart(struct ScriptContext *ctx)
{
    CreateSlateShopMenu();
    ScriptContext1_Stop();
    return TRUE;
