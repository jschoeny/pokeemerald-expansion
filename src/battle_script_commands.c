            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                    ...
                    else if (gBattleMons[gBattlerTarget].item == ITEM_ENIGMA_BERRY_E_READER
                    ...
                    else if(gBattleMons[gBattlerAttacker].item != 0)
                    {
                        if(GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER
                            && (gBattleTypeFlags & BATTLE_TYPE_TRAINER) == FALSE)
                        {
                            StealTargetItem(gBattlerAttacker, gBattlerTarget);  // Attacker steals target item
                            AddBagItem(gLastUsedItem, 1);
                            BattleScriptPush(gBattlescriptCurrInstr + 1);
                            gBattlescriptCurrInstr = BattleScript_ItemStealBag;
                        }
                        else
                            gBattlescriptCurrInstr++;
                    }
...
static void Cmd_getexp(void)
{
            ...
            if (!gSaveBlock2Ptr->expShare && !(gBattleStruct->sentInPokes & 1))
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                ...
                gBattleMoveDamage = 0; // used for exp
                  ...
                  || GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) >= GetLevelCap(FALSE))
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) < GetLevelCap(FALSE))
            {
