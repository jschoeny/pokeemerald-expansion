...
static void PrintMonAbilityName(void)
{
    u16 ability = GetAbilityBySpeciesPersonality(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum, sMonSummaryScreen->summary.pid);
...
static void PlayMonCry(void)
{
    ...
    if (!summary->isEgg)
    {
        if (ShouldPlayNormalMonCry(&sMonSummaryScreen->currentMon) == TRUE)
            PlayCry_ByMode_Personality(summary->species2, 0, CRY_MODE_NORMAL, summary->pid);
        else
            PlayCry_ByMode_Personality(summary->species2, 0, CRY_MODE_WEAK, summary->pid);
    }
}
