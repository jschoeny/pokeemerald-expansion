...
static void PrintInstructionsOnWindow(struct PokemonDebugMenu *data)
{
    u8 fontId = 0;
    u8 x = 2;
    u8 textInstructions[] = _("{START_BUTTON} Shiny {SELECT_BUTTON} Give\n{B_BUTTON} Exit  {A_BUTTON} Submenu 1$");
...
static void Handle_Input_Debug_Pokemon(u8 taskId)
{
    ...
    if (JOY_NEW(SELECT_BUTTON)) {
        if (data->currentSubmenu == 0) {
            ScriptGiveMon(data->currentmonId, 90, ITEM_NONE, 0, 0, 0);
            PlaySE(SE_SAVE);
        }
        else if (gBaseStats[data->currentmonId].flags & FLAG_GENDER_DIFFERENCE)
        {
