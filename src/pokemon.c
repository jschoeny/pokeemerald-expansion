...
static const u16 sSpeciesToHoennPokedexNum[NUM_SPECIES - 1] =
{
    ...
    // Galarian Forms
    [SPECIES_CORSOLA_GALARIAN - 1] = HOENN_DEX_CORSOLA,
    [SPECIES_ZIGZAGOON_GALARIAN - 1] = HOENN_DEX_ZIGZAGOON,
    [SPECIES_LINOONE_GALARIAN - 1] = HOENN_DEX_LINOONE,
    // Shellos
    ...
    // Mimikyu
    [SPECIES_MIMIKYU_BUSTED - 1] = HOENN_DEX_MIMIKYU,
};
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            ...
            if ((itemEffect[i] & ITEM3_LEVEL_UP)
             && GetMonData(mon, MON_DATA_LEVEL, NULL) < GetLevelCap(TRUE))
            {
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (expPoints > gExperienceTables[gBaseStats[species].growthRate][GetLevelCap(FALSE)])
    {
        expPoints = gExperienceTables[gBaseStats[species].growthRate][GetLevelCap(FALSE)];
        SetMonData(mon, MON_DATA_EXP, &expPoints);
    }
    if (nextLevel > GetLevelCap(FALSE) || expPoints < gExperienceTables[gBaseStats[species].growthRate][nextLevel])
    {
        return FALSE;
    }
    else
...
{
    return GetLevelCapByOption(isRareCandy, 0xFF);
}
...
u8 GetLevelCapByOption(bool8 isRareCandy, u8 option)
{
    ...
    if(option == 0xFF)
        option = gSaveBlock2Ptr->optionsChallengeLevelCap;
    ...
    if(option == OPTIONS_RANDOMIZER_LEVELCAP_CANDY && isRareCandy)
        levelCap = CHALLENGE_LEVEL(badgeLevelCaps[gSaveBlock2Ptr->optionsChallengeStrongerTeams][nBadges]);
    else if(option == OPTIONS_RANDOMIZER_LEVELCAP_NORMAL)
        levelCap = CHALLENGE_LEVEL(badgeLevelCaps[gSaveBlock2Ptr->optionsChallengeStrongerTeams][nBadges]);
    else if(option == OPTIONS_RANDOMIZER_LEVELCAP_HARD)
