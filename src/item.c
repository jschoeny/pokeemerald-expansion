...
static bool8 CheckPyramidBagHasSpace(u16 itemId, u16 count)
{
    ...
    for (i = 0; i < PYRAMID_BAG_ITEMS_COUNT; i++)
    {
        ...
        {
            if (quantities[i] + count <= MAX_PBAG_ITEM_CAPACITY)
                return TRUE;

            count = (quantities[i] + count) - MAX_PBAG_ITEM_CAPACITY;
            if (count == 0)
                return TRUE;
        }
    }
...
bool8 AddPyramidBagItem(u16 itemId, u16 count)
{
    ...
    for (i = 0; i < PYRAMID_BAG_ITEMS_COUNT; i++)
    {
        if (newItems[i] == itemId && newQuantities[i] < MAX_PBAG_ITEM_CAPACITY)
        {
            newQuantities[i] += count;
            if (newQuantities[i] > MAX_PBAG_ITEM_CAPACITY)
            {
                count = newQuantities[i] - MAX_PBAG_ITEM_CAPACITY;
                newQuantities[i] = MAX_PBAG_ITEM_CAPACITY;
            }
            else
            {
                count = 0;
