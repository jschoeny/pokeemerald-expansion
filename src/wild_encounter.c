...
bool8 BuildWildMonForOutbreak(u16 location, u8 *level, u16 *species, u16 *eggMove, u8 *isWaterMon)
{
    ...
    *eggMove = 0;
    ...
    // Land Pokemon
        ...
        *level = landMonsInfo->wildPokemon[timeOfDay][monIndex].maxLevel + 4;
        *species = landMonsInfo->wildPokemon[timeOfDay][monIndex].species;
        ...
        if(FlagGet(FLAG_BADGE05_GET)) {
            *isWaterMon = TRUE;
            if(fishingMonsInfo == NULL || Random() % 100 < 60) { // Surfing Mon
                ...
                *level = waterMonsInfo->wildPokemon[timeOfDay][monIndex].maxLevel + 4;
                *species = waterMonsInfo->wildPokemon[timeOfDay][monIndex].species;
                *eggMove = GetRandomEggMoveSpecies(*species);
            }
            else if(fishingMonsInfo != NULL) { // Fishing Mon
                monIndex = ChooseWildMonIndex_Fishing(SUPER_ROD);
                *level = fishingMonsInfo->wildPokemon[timeOfDay][monIndex].maxLevel + 4;
                *species = fishingMonsInfo->wildPokemon[timeOfDay][monIndex].species;
                *eggMove = GetRandomEggMoveSpecies(*species);
            }
        }
        else {
